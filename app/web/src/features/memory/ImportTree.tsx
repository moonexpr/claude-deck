  ...
  return (
    <div className="select-none">
      ...
      >
        ...
        <span
          className={`font-mono text-sm ${!node.exists ? "text-destructive" : ""}`}
        >
          {fileName}
        </span>
...
export function ImportTree({ files }: ImportTreeProps) {
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to resolve imports"
      );
  ...
  const countImports = (
    node: ImportTreeNode
  ): { total: number; missing: number; cycles: number } => {
    let total = 0;
    let missing = 0;
    let cycles = 0;
