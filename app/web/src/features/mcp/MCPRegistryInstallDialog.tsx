...
import { useMCPRegistryApi } from "./useMCPRegistryApi";
...
export function MCPRegistryInstallDialog({
  ...
  const selectedTransport: TransportOption | undefined =
    transportOptions[selectedTransportIdx];
  ...
  const getSelectedEnvVars = () => {
    ...
    if (
      selectedTransport.kind === "package" &&
      selectedTransport.packageIndex !== undefined
    ) {
      return (
        server.packages?.[selectedTransport.packageIndex]?.environmentVariables || []
      );
    }
    return [];
  };
  ...
  const getSelectedArgs = () => {
    ...
    if (
      ...
      return (
        server.packages?.[selectedTransport.packageIndex]?.packageArguments || []
      );
    }
    return [];
  };
  ...
  const getSelectedHeaders = () => {
    if (!server || !selectedTransport) return [];
    if (
      selectedTransport.kind === "remote" &&
      selectedTransport.remoteIndex !== undefined
    ) {
  ...
  const effectiveStep = hasTransportChoice ? step : step + 1;
  ...
  const buildRequest = (): RegistryInstallRequest | null => {
    ...
    const request: RegistryInstallRequest = { server_name: serverName, scope };
  ...
  const getPreviewConfig = (): Record<string, unknown> => {
    ...
    if (
      ...
      if (pkg.registryType === "npm") {
        if (command === "npx") cmdArgs.push("-y");
        cmdArgs.push(pkg.version ? `${pkg.identifier}@${pkg.version}` : pkg.identifier);
      } else if (pkg.registryType === "pypi") {
        cmdArgs.push(pkg.identifier);
      } else if (pkg.registryType === "oci") {
        cmdArgs.push("run", "-i", "--rm", pkg.identifier);
      }

      for (const [name, value] of Object.entries(argValues)) {
        if (value.trim()) cmdArgs.push(`--${name}`, value);
      }

      const config: Record<string, unknown> = { type: "stdio", command, args: cmdArgs };
        ...
        <div className="space-y-4 py-2">
          ...
          {effectiveStep === 3 && (
            <div className="space-y-4">
              ...
              <Select
                value={scope}
                onValueChange={(v: "user" | "project") => setScope(v)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
