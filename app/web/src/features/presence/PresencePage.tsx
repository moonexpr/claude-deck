  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      >
        ...
        <RefreshButton onClick={() => void loadSessions()} loading={loading} />
        <Button
      ...
      {sortedSessions.length > 0 && (
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => void handleClearStopped()}
            disabled={!sortedSessions.some((s) => s.status === 'stopped' || s.status === 'idle')}
          >
            Clear stopped
          </Button>
          <Button variant="outline" size="sm" onClick={() => void handleClearAll()}>
      ...
      {sortedSessions.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedSessions.map((s) => (
            <PresenceCard key={s.session_id} session={s} onRemove={(id) => void handleRemove(id)} />
          ))}
        </div>
      ) : (
        !loading && (
