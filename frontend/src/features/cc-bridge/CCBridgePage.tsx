...
export function CCBridgePage() {
  ...
  const gridCols = activeTargets.length <= 1 ? 'grid-cols-1' : (activeTargets.length === 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-2')
      ...
      {!isFullscreen && (
        <div className="flex items-center gap-3 px-3 py-2 md:px-4 md:py-3 border-b shrink-0 bg-muted/30">
          ...
          <div className="flex items-baseline gap-2 flex-wrap min-w-0 overflow-hidden">
            <h1 className="text-sm md:text-base font-semibold truncate">CC Bridge</h1>
      ...
      <div className="flex flex-1 min-h-0 flex-col md:flex-row">
        {!isFullscreen && (
          <div className={cn(
            ...
            sidebarOpen ? "w-full md:w-52 border-b md:border-b-0 h-[200px] md:h-auto" : "w-0 h-0 border-0"
          )}>
            <SessionList
              sessions={sessions}
              loading={loading}
